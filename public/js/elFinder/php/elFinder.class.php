...
class elFinder
{
    ...
    protected $commands = [
        ...
        'tmb' => ['targets' => true],
        'file' => ['target' => true, 'download' => false, 'cpath' => false],
    ...
    public function __construct($opts)
    {
        ...
        if (version_compare(PHP_VERSION, '5.6', '>=')) {
            if (($_val = ini_get('iconv.internal_encoding')) && 'UTF-8' !== strtoupper($_val)) {
                ini_set('iconv.internal_encoding', '');
            }
            if (($_val = ini_get('mbstring.internal_encoding')) && 'UTF-8' !== strtoupper($_val)) {
                ini_set('mbstring.internal_encoding', '');
            }
            if (($_val = ini_get('internal_encoding')) && 'UTF-8' !== strtoupper($_val)) {
                ini_set('internal_encoding', '');
            }
        } else {
            if (function_exists('iconv_set_encoding') && 'UTF-8' !== strtoupper(iconv_get_encoding('internal_encoding'))) {
                iconv_set_encoding('internal_encoding', 'UTF-8');
            }
            if (function_exists('mb_internal_encoding') && 'UTF-8' !== strtoupper(mb_internal_encoding())) {
    ...
    protected function zipdl($args)
    {
        ...
        if (!$download) {
            ...
            if (4 !== count($targets) || ($volume = $this->volume($targets[0])) == false || !($file = $this->session->get('zipdl'.$targets[1]))) {
    ...
    protected function rename($args)
    {
        $target = $args['target'];
        $name = $args['name'];
        $query = (false !== strpos($args['q'], '*')) ? $args['q'] : '';
        $targets = $args['targets'];
        $rms = [];
        $notfounds = [];
        $locked = [];
    ...
    {
        if (!elFinder::$connectionFlagsPath || 'HEAD' === $_SERVER['REQUEST_METHOD']) {
            return;
        }
    ...
    protected function upload($args)
    {
        ...
        foreach ($files['name'] as $i => $name) {
            ...
            // Set name if name eq 'image.png' and $args has 'name' array, e.g. clipboard data
            if ('image' === strtolower(substr($name, 0, 5)) && is_array($args['name']) && isset($args['name'][$i])) {
                $type = $files['type'][$i];
                $name = $args['name'][$i];
    ...
    protected function get($args)
    {
        ...
        if ($mime && ('text' === strtolower(substr($mime, 0, 4)) || in_array(strtolower($mime), self::$textMimes))) {
            $enc = '';
            if ('' !== $content) {
        ...
        if (false !== $enc) {
            ...
            if (false === $content || false === $json || strlen($json) < strlen($content)) {
    ...
    protected function getNetVolumeUniqueId($netVolumes = null, $prefix = 'nm')
    {
        ...
        foreach ($netVolumes as $vOps) {
            if (isset($vOps['id']) && 0 === strpos($vOps['id'], $prefix)) {
                $ids[$vOps['id']] = true;
            }
        }
        if (!$ids) {
    ...
    public static function isAnimationPng($path)
    {
        ...
        fclose($fp);
        if ($img_bytes) {
            if (false !== strpos(substr($img_bytes, 0, strpos($img_bytes, 'IDAT')), 'acTL')) {
                return true;
            }
        }
    ...
    {
        if (0 != elFinder::$memoryLimitGD && $imgInfos && is_array($imgInfos)) {
