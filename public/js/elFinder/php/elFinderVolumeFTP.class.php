...
class elFinderVolumeFTP extends elFinderVolumeDriver
{
    ...
    protected function connect()
    {
        ...
        $this->isPureFtpd = false !== stripos(implode(' ', $help), 'Pure-FTPd');
    ...
    protected function parseRaw($raw, $base, $nameOnly = false)
    {
        ...
        if (is_numeric($info[5]) && !$info[6] && !$info[7]) {
            ...
            if ($stat['ts'] && $stat['ts'] > $now && false !== strpos($info[7], ':')) {
        ...
        $stat['name'] = $name;
        $stat['mime'] = 'd' == substr(strtolower($info[0]), 0, 1) ? 'directory' : $this->mimetype($stat['name'], true);
     ...
     * @param bool   $isowner. Tell if the current user is the owner of the object.
     *
     * @return string
     *
     * @author Dmitry (dio) Levashov
    ...
    protected function parsePermissions($perm, $isowner = true)
    {
        ...
        $read = ($isowner && 'r' == $parts[1]) || 'r' == $parts[4] || 'r' == $parts[7];

        return [
            'read' => 'd' == $parts[0] ? $read && (($isowner && 'x' == $parts[3]) || 'x' == $parts[6] || 'x' == $parts[9]) : $read,
            'write' => ($isowner && 'w' == $parts[2]) || 'w' == $parts[5] || 'w' == $parts[8],
        ];
    }
    ...
    {
        return $path == $parent || 0 === strpos($path, rtrim($parent, $this->separator).$this->separator);
    }
    ...
    protected function _stat($path)
    {
        ...
        if (is_array($raw) && count($raw) > 1 && 2 == substr(trim($raw[0]), 0, 1)) {
            ...
            foreach ($parts as $part) {
                ...
                switch ($key) {
                    case 'type':
                        if (false !== strpos($val, 'dir')) {
                            $stat['mime'] = 'directory';
                        } elseif (false !== strpos($val, 'link')) {
                            $stat['mime'] = 'symlink';
                            break 2;
                        } else {
            ...
            if ($mode) {
                ...
                $stat['read'] = 'directory' == $stat['mime'] ? $read && (($owner_computed && $perm[0][2]) || $perm[1][2] || $perm[2][2]) : $read;
