...
class elFinderVolumeOneDrive extends elFinderVolumeDriver
{
    ...
    public function getContentUrl($hash, $options = [])
    {
        ...
        $res = '';
        if (($file = $this->file($hash)) == false || !$file['url'] || 1 == $file['url']) {
        ...
        try {
            ...
            if (preg_match('/Location: (.+)/', $result, $m)) {
                ...
                while ($res) {
                    ...
                    curl_close($curl);
                    if (isset($res->status)) {
                        if ('completed' === $res->status || 'failed' === $res->status) {
                            break;
                        }
    ...
    protected function _save($fp, $path, $name, $stat)
    {
        ...
        if ('' === $name) {
            ...
            if ($stat) {
                ...
                if (isset($stat['rev']) && 0 === strpos($stat['hash'], $this->id)) {
                    $itemId = $stat['rev'];
                }
            }
